...
import pacman.model.entity.dynamic.ghost.newGhost.Blinky;
import pacman.model.entity.dynamic.ghost.newGhost.Inky;
import pacman.model.entity.dynamic.ghost.newGhost.Pinky;
...
public class Maze {
    ...
    private final Map<Character, Renderable> ghosts; // Changed from List to Map to store different ghost types
    private final List<Renderable> genericGhosts;
    ...
    public Maze() {
        ...
        this.genericGhosts = new ArrayList<>();
    ...
    public void addRenderable(Renderable renderable, char renderableType, int x, int y) {
        if (renderable != null) {
            ...
            } else if (renderableType == RenderableType.BLINKY_GHOST) {
                this.ghosts.put(RenderableType.BLINKY_GHOST, renderable);
            } else if (renderableType == RenderableType.PINKY_GHOST) {
                this.ghosts.put(RenderableType.PINKY_GHOST, renderable);
            } else if (renderableType == RenderableType.INKY_GHOST) {
                this.ghosts.put(RenderableType.INKY_GHOST, renderable);
            } else if (renderableType == RenderableType.CLYDE_GHOST) {
                this.ghosts.put(RenderableType.CLYDE_GHOST, renderable);
            } else if (renderableType == RenderableType.GHOST) {
                this.genericGhosts.add(renderable);
    ...
    public List<Renderable> getGhosts() {
        List<Renderable> allGhosts = new ArrayList<>(ghosts.values());
        allGhosts.addAll(genericGhosts);
        return allGhosts;
    }

    public void setInky() {
        Renderable blinky = ghosts.get(RenderableType.BLINKY_GHOST);
        Inky inky = (Inky) ghosts.get(RenderableType.INKY_GHOST);
        if (blinky != null && inky != null) {
            inky.setShadowGhost(blinky);
        }
    }
    ...
    public List<Renderable> getPowerPellets() { return powerPellets; }
