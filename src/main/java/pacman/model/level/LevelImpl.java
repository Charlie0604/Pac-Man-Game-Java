...
public class LevelImpl implements Level {
    ...
    private boolean frightedState;
    private int frightedCount;
    ...
    public LevelImpl(JSONObject levelConfiguration,
        ...
        this.points = 0;
        this.eatCount = 0;
        this.frightedState = false;
        this.frightedCount = 0;
    ...
    public void tick() {
        if (this.gameState != GameState.IN_PROGRESS) {
            ...
            // enter frighted state
            if (frightedState){

                // time up
                if (frightedCount == modeLengths.get(GhostMode.FRIGHTENED) * 30) {
                    for (Ghost ghost : ghosts) {
                        if (ghost.isFrightenedState()){
                            ghost.setState('n');
                        }
                    }
                    frightedState = false;
    ...
    @Override
    public boolean isPower(Renderable renderable) {
        return maze.getPowerPellets().contains(renderable) && ((Collectable) renderable).isCollectable();
    }
